...
techs: ["Cisco", "VLAN", "Trunk", "SVI", "Layer 3 Switch", "Packet Tracer"]
date: "2026-06-08"
status: "Terminé"
---
...
Ce compte-rendu présente la mise en œuvre du routage inter-VLAN sur des équipements Cisco IOS dans Packet Tracer, puis son déploiement sur une maquette réelle en laboratoire. L'objectif est de segmenter un réseau local en trois VLANs distincts et d'assurer la communication entre eux à l'aide d'un commutateur de couche 3 (Multilayer Switch) exploitant des interfaces virtuelles (SVI).

---

## 💡 1. Principes théoriques & Architecture

### 💡 L'explication vulgarisée

Par défaut, un VLAN (Virtual LAN) forme une frontière étanche : les machines du VLAN 10 ne peuvent pas parler à celles du VLAN 20. C'est parfait pour la sécurité, mais problématique si un ordinateur doit joindre un serveur situé ailleurs.

Pour franchir ces frontières, il faut un équipement de niveau 3 (Réseau) capable de router les paquets :

- **La méthode classique ("Router on a Stick")** : On envoie tout le trafic via un lien unique (Trunk) vers un routeur externe qui fait l'aiguillage. C'est efficace mais cela crée un goulot d'étranglement.
- **La méthode moderne (Switch de couche 3)** : On utilise un switch haut de gamme capable de router lui-même les paquets à la vitesse du matériel. On crée des SVI (Switch Virtual Interfaces), des routeurs virtuels internes au switch qui servent de passerelles pour chaque VLAN.
