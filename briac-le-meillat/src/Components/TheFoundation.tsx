        ...
        <section id="the-foundation" className="w-full py-32 bg-black border-t border-white/5 relative overflow-hidden">
            <div className="max-w-7xl mx-auto px-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-20">
                    {pillars.map((pillar, index) => (
                        <motion.div
                            ...
                            viewport={{ once: true }}
                            className="flex flex-col items-center text-center space-y-8 p-10 rounded-[2.5rem] bg-zinc-900/20 border border-white/5 hover:border-white/10 transition-all duration-700 group"
                        >
                            ...
                            <div className="space-y-3">
                                ...
                                <p className="text-[10px] font-mono text-white/30 uppercase tracking-[0.3em] group-hover:text-blue-500 transition-colors duration-500">
                                    {pillar.tagline}
                                </p>
                            </div>
                        </motion.div>
                ...
                <div className="flex flex-col items-center">
                    ...
                    >
                        <div className={`w-16 h-16 rounded-full border border-white/10 flex items-center justify-center transition-all duration-500 group-hover:border-blue-500/50 group-hover:bg-blue-500/5 ${isExpanded ? 'bg-white text-black' : 'bg-transparent text-white'}`}>
                            {isExpanded ? <Minus size={24} strokeWidth={1.5} /> : <Plus size={24} strokeWidth={1.5} />}
                        </div>
                        <span className="text-[11px] font-['Paris2024'] uppercase tracking-[0.4em] text-white/20 group-hover:text-white transition-colors">
                            {isExpanded ? "Replier L'Architecture" : "Explorer les Fondations Académiques"}
                        </span>
                    </button>
                    ...
                    <AnimatePresence>
                        {isExpanded && (
                            <motion.div
                                ...
                                transition={{ duration: 1, ease: APPLE_BEZIER as any }}
                                className="w-full overflow-hidden mt-20"
                            >
                                <div className="pt-16 border-t border-white/5 px-4">
                                    <CompetencesBUT />
                                </div>
                            </motion.div>
                        )}
