        ...
        >
            ...
            */}
            <div style={{
                ...
                {/* Main Title: Bérangère • Production */}
...
/* ─────────────────────────────────────────────
   FILM CARD (portrait, poster above white card)
───────────────────────────────────────────── */
function FilmCard({ project }: { project: Film }) {
...
/* ─────────────────────────────────────────────
   SERIES CARD (16/9 landscape with overlay)
───────────────────────────────────────────── */
function SeriesCard({ project }: { project: Serie }) {
    const [hovered, setHovered] = useState(false);
    const basePath = import.meta.env.BASE_URL || '/';
    ...
    const handleClick = () => {
        ...
        window.open(`${base}/berangere/serie/${project.id}`, '_blank');
    };
    ...
    return (
        ...
        >
            ...
            <div style={{
                ...
                {project.series && project.series[0] && (
                    <p style={{
                        fontSize: '0.6rem',
                        letterSpacing: '0.2em',
                        textTransform: 'uppercase',
                ...
                <div style={{
                    ...
                    <div style={{ flex: 1, paddingTop: '0.5rem' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            {[
                                { label: 'Réalisatrice', value: featuredFilm.director || 'À compléter' },
                                { label: 'Distribution', value: featuredFilm.cast || 'À compléter' },
                                { label: 'Production', value: featuredFilm.production || 'À compléter' },
                                { label: 'Durée', value: featuredFilm.duration || 'À compléter' },
                            ].map(({ label, value }) => (
                                <div key={label}>
...
function SeriesSection() {
    ...
    const featuredEpisode = useMemo(() =>
        [...series].sort((a, b) => (b.createdAt as unknown as number) - (a.createdAt as unknown as number))[0],
        [series]
    );

    const allSeriesLabels = Array.from(new Set(series.flatMap(s => s.series ?? []))).filter(Boolean) as string[];

    const filtered = useMemo(() => {
        let list = series;
        if (filter !== 'ALL') list = list.filter(s => s.series?.includes(filter));
        if (search.trim()) {
            const q = search.toLowerCase();
    ...
    return (
        ...
        >
            ...
            <div style={{
                ...
                {/* Left: large avatar circle + BriBri */}
                <div style={{
                    flexShrink: 0,
                    display: 'flex',
                    flexDirection: 'column',
                ...
                <div style={{ flex: 1, paddingTop: '0.25rem' }}>
                    ...
                    <p style={{
                        ...
                        Depuis plusieurs années, YouTube a été le premier espace où j'ai pu donner vie à mes réalisations documentaires. Un terrain d'exploration libre, sans contrainte de diffusion, où chaque série est née d'une curiosité sincère pour l'Histoire, les légendes et les destins humains.
                    </p>
                    <p style={{
                        fontSize: '0.82rem',
                        color: '#aaa',
                        ...
                        [Votre texte à compléter — parlez de votre démarche éditoriale, de ce qui vous pousse à raconter ces histoires, ou de la vision qui anime chacune de vos séries.]
                    </p>
                </div>
            </div>

            {/* ── Bloc 2 : Dernier épisode ── */}
            {featuredEpisode && (
                <div style={{
                    display: 'flex',
                    gap: 'clamp(2rem, 5vw, 4rem)',
                    ...
                    {/* Episode info */}
                    <div style={{ flex: 1, paddingTop: '0.5rem' }}>
                        {/* "dernier épisode" in Book Italic */}
                        <p style={{
                            fontFamily: "'NeutrafaceText', sans-serif",
                            fontWeight: 400,
                            fontStyle: 'italic',
                        ...
                        {/* Series / category tag */}
                        {(featuredEpisode.series?.[0] || featuredEpisode.category) && (
                            <p style={{
                                fontFamily: "'NeutrafaceText', sans-serif",
                                fontWeight: 400,
                                fontStyle: 'italic',
                                ...
                                {featuredEpisode.series?.[0]}{featuredEpisode.category ? ` · ${featuredEpisode.category}` : ''}
                            </p>
                        )}
                        {/* Separator */}
                        <div style={{ width: '2rem', height: '1px', background: '#ccc', marginBottom: '1.5rem' }} />
                        {/* Info rows */}
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '1.5rem' }}>
                            {[
                                { label: 'Saison', value: `${featuredEpisode.season}` },
                                { label: 'Épisode', value: `${featuredEpisode.episode}` },
                                { label: 'Durée', value: featuredEpisode.duration },
            ...
            <div style={{
                ...
                <span style={{ fontSize: '0.7rem', letterSpacing: '0.18em', color: '#bbb', textTransform: 'uppercase' }}>
                    Bérangère — Production Cinématographique
                </span>
                <Link
                    to="/"
                    style={{
